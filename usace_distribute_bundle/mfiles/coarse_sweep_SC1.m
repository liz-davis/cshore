...
outdir        = fullfile(base_data_dir, 'generated_infiles_sweep_coarse');
...
slp_vals   = [0.9, 1.2, 1.4];
gamma_vals = [0.30, 0.35, 0.40];
efff_vals  = [0.014, 0.018, 0.020];
tanphi_vals  = [0.60, 0.63];
fric_fac_vals    = [0.006, 0.008];
...
for i1 = 1:numel(efff_vals)
    for i2 = 1:numel(slp_vals)
        for i3 = 1:numel(gamma_vals)
            for i4 = 1:numel(fric_fac_vals)
                for i5 = 1:numel(tanphi_vals)

                    combo_id = combo_id + 1;

                    combo_table(combo_id).combo_id  = combo_id;
                    combo_table(combo_id).effb      = effb_fixed;
                    combo_table(combo_id).efff      = efff_vals(i1);
                    combo_table(combo_id).slp       = slp_vals(i2);
                    combo_table(combo_id).gamma     = gamma_vals(i3);
                    combo_table(combo_id).fric_fac  = fric_fac_vals(i4);
                    combo_table(combo_id).tanphi    = tanphi_vals(i5);
                    combo_table(combo_id).blp       = blp_fixed;
...
disp('Coarse parameter sweep infile generation complete.');