function in=set_defaults

% CSHORE execution and physical params
in.header = {'------------------------------------------------------------'
             'CSHORE model run defaults'
             '------------------------------------------------------------'};
...
in.dx     = 1;          % constant dx 
in.gamma  = .3;         % shallow water ratio of wave height to water depth
in.sporo  = 0.4;        % sediment porosity                        
in.d50 = .3;            % d_50 in mm
in.wf = vfall(in.d50,20,0); % fall velocity
in.sg = 2.65;           % specific gravity
in.effb   = 0.0070;     % suspension efficiency due to breaking eB     
in.efff   = 0.018;      % suspension efficiency due to friction ef 
in.slp    = .9;         % suspended load parameter               
in.slpot  = .1;         % overtopping suspended load parameter               
in.tanphi = .630;       % tangent (sediment friction angle)        
in.blp    = 0.0015;     % bedload parameter                        
